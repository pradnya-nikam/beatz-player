...
public class EnergyOutputAudioDevice extends BaseOutputAudioDevice {
    ...
    private Queue<Short> instantBuffer = new ArrayDeque<Short>(4096);
    ...
    protected void outputImpl(short[] samples, int offs, int len) throws JavaLayerException {
        ...
        int minBufferSize = averageLength * channels;
        while(instantBuffer.size()> minBufferSize)
        {
            long energy = 0;
            for(int i=0; i< minBufferSize; i++){
                short sample = instantBuffer.poll();
